...
@Component({
  ...
  standalone: true,
  templateUrl: './learner-license.component.html',
  styleUrls: ['./learner-license.component.css'],
  imports: [BackBtnComponent, CommonModule, FormsModule, ReactiveFormsModule],
})
export class LearnerLicenseComponent {
  stepIndex = 0;
  ...
  onAuthenticateSubmit() {
    if (this.authForm.valid) {
      console.log('Authentication Form Values:', this.authForm.value);
      this.stepIndex = 2;
    } else {
 ...
 submitApplication() {
  ...
  formData.append('dateOfBirth', this.authForm.value.dateOfBirth);
  ...
  this.http.post('http://localhost:3000/api/learner-license', formData).subscribe({
    next: (res: any) => {
